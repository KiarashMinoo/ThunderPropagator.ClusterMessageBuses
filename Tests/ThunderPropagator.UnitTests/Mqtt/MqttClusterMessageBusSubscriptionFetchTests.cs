...
public class MqttClusterMessageBusSubscriptionFetchTests
{
    [Fact]
    public async Task BuildResponseAsync_FetchSubscriptions_ReturnsTheResolvedChannelsLocalDescriptors()
    {
        ...
        var request = new MqttClusterRequestEnvelope(Guid.NewGuid(), MqttClusterRequestKind.FetchSubscriptions, null, channelKey, null, new Uri("https://requester:5001/"));
    ...
    public async Task FetchPeerSubscriptionsAsync_MatchingReplyArrives_ReturnsTheDescriptors()
    {
        ...
        var sentRequest = publishedJson.FromNJson<MqttClusterRequestEnvelope>()!;

        var descriptors = new[] { new ClusterSubscriptionDescriptor("sub-9", "req-9", "conn-9") };
        bus.TryCompletePendingRequest(new MqttClusterResponseEnvelope(sentRequest.CorrelationId, true, null, descriptors.ToNJson()));
