...
namespace ThunderPropagator.ClusterMessageBuses.Rdma
{
    ...
    /// plus <c>PullSnapshotAsync</c> in <c>RdmaClusterMessageBus.BytePayload.cs</c>. Requester side:
    /// <see cref="SendRequestAsync"/> sends a <see cref="ClusterRequestEnvelope"/> over this
    /// node's own outbound queue pair to the target peer and awaits a matching
    /// <see cref="ClusterResponseEnvelope"/> -- which arrives back over that exact same queue
    /// pair rather than a separate reply channel (RDMA's RC mode has no separate reply channel:
    /// since the connection is inherently bidirectional, the answering side simply writes its
    /// response back over the exact same queue pair the request arrived on). Answering side: <see cref="HandleRequestFrameAsync"/> dispatches to
    /// <see cref="BuildResponseAsync"/>, implemented per request kind in
    /// <c>RdmaClusterMessageBus.Snapshots.cs</c> / <c>.SubscriptionFetch.cs</c> / <c>.BytePayload.cs</c>,
    /// then writes the response back via the caller-supplied <c>sendResponseAsync</c> delegate.
    /// Mirrors <c>TcpClusterMessageBus.RequestReply.cs</c> exactly, including reusing
    /// <see cref="ClusterResiliencePipelineFactory"/> for the retry/circuit-breaker around the send
    /// itself -- RDMA's RC mode is reliable and ordered, exactly like TCP, so the same reasoning
    /// (retry only the send, not the full send-then-await-reply round trip) applies unchanged.
    /// </summary>
    internal sealed partial class RdmaClusterMessageBus
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var frame = new RdmaClusterFrame(RdmaClusterFrameKind.Request, request.ToNJson());
                ...
                // (up to RequestTimeout per attempt), same reasoning as the TCP transport.
                ...
                if (!response.Success)
                    throw new InvalidOperationException($"RDMA cluster request '{kind}' to '{targetPeerEndpoint.Host}' was rejected: {response.ErrorMessage}");
            ...
            {
                throw new TimeoutException($"RDMA cluster request '{kind}' to '{targetPeerEndpoint.Host}' timed out after {_options.RequestTimeout}.");
            }
            finally
            {
        ...
        /// invoking <paramref name="sendResponseAsync"/> with the resulting <see cref="RdmaClusterFrame"/>.
        ...
        internal async Task HandleRequestFrameAsync(
            string payload,
            Func<RdmaClusterFrame, CancellationToken, Task> sendResponseAsync,
            CancellationToken cancellationToken)
        {
            ...
            var responseFrame = new RdmaClusterFrame(RdmaClusterFrameKind.Response, response.ToNJson());
        ...
        {
            [LoggerMessage(EventId = 91730, Level = LogLevel.Warning,
                Message = "[Cluster] RDMA cluster request could not be parsed; skipping it.")]
            public static partial void RequestUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91731, Level = LogLevel.Warning,
                Message = "[Cluster] RDMA cluster response could not be parsed; skipping it.")]
            public static partial void ResponseUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91732, Level = LogLevel.Warning,
                Message = "[Cluster] Sending an RDMA cluster response back to the requester failed.")]
            public static partial void ResponseSendFailed(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91733, Level = LogLevel.Error,
                Message = "[Cluster] RDMA request handling faulted for kind '{Kind}'.")]
