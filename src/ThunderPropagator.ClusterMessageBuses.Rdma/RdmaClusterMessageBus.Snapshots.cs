...
namespace ThunderPropagator.ClusterMessageBuses.Rdma
{
    internal sealed partial class RdmaClusterMessageBus
    {
        ...
        {
            [LoggerMessage(EventId = 91740, Level = LogLevel.Information,
                Message = "[Cluster] RDMA restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91741, Level = LogLevel.Information,
                Message = "[Cluster] RDMA restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91742, Level = LogLevel.Debug,
                Message = "[Cluster] RDMA delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
