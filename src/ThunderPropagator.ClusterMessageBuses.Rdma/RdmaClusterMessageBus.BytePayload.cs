...
{
    /// <summary>
    /// Byte-oriented counterpart to <c>RdmaClusterMessageBus.FanOut.cs</c> /
    ...
    /// Fan-out is multiplexed over the same physical queue pair as every other concern, using the
    /// <see cref="RdmaClusterFrameKind.ByteFanOut"/> discriminator and a
    /// <see cref="ClusterChannelEnvelope{TMessage}"/> wrapper -- mirroring how the same envelope
    /// type wraps the channel key alongside a <c>ClusterFanOutMessage</c> for plain fan-out. The
    /// snapshot pull reuses the existing hand-rolled request/reply plumbing in
    /// <c>RdmaClusterMessageBus.RequestReply.cs</c> via the
    ...
    internal sealed partial class RdmaClusterMessageBus
    {
        ...
        public override async Task PublishAsync(Guid channelKey, ClusterByteMessage message, CancellationToken cancellationToken = default)
        {
            ...
            var frame = new RdmaClusterFrame(RdmaClusterFrameKind.ByteFanOut, new ClusterChannelEnvelope<ClusterByteMessage>(channelKey, stamped).ToNJson());
            ...
            await Task.WhenAll(peers.Select(async peer =>
            {
                ...
                catch (Exception exception)
                {
                    ...
                    // ClusterFanOutMessage overload in RdmaClusterMessageBus.FanOut.cs.
        ...
        {
            [LoggerMessage(EventId = 91760, Level = LogLevel.Warning,
                Message = "[Cluster] RDMA byte fan-out send to peer '{Host}' failed.")]
            public static partial void ByteFanOutSendToPeerFailed(ILogger logger, Exception exception, string host);

            [LoggerMessage(EventId = 91761, Level = LogLevel.Warning,
                Message = "[Cluster] RDMA byte fan-out message could not be parsed; skipping it.")]
            public static partial void ByteFanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91762, Level = LogLevel.Error,
                Message = "[Cluster] RDMA byte fan-out handler faulted.")]
