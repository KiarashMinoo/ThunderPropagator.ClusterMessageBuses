    ...
    internal sealed partial class RabbitMqClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, RabbitMqClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, RabbitMqClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);

            var delta = response.PayloadJson?.FromNJson<RabbitMqSnapshotDeltaPayload>();
            if (delta is null)
                return;
        ...
        private async Task<RabbitMqClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(RabbitMqClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new RabbitMqClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<RabbitMqClusterResponseEnvelope> BuildSyncDeltaResponseAsync(RabbitMqClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new RabbitMqSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new RabbitMqClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9640, Level = LogLevel.Information,
                Message = "[Cluster] RabbitMQ restoring channel '{Channel}' from leader host '{Host}'.")]
            ...
            [LoggerMessage(EventId = 9641, Level = LogLevel.Information,
                Message = "[Cluster] RabbitMQ restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 9642, Level = LogLevel.Debug,
                Message = "[Cluster] RabbitMQ delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
