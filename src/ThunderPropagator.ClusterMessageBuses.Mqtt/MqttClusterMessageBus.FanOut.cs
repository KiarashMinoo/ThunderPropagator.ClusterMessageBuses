...
namespace ThunderPropagator.ClusterMessageBuses.Mqtt
{
    internal sealed partial class MqttClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterFanOutMessage message, CancellationToken cancellationToken = default)
        {
            ...
            var topic = MqttTopicNaming.FanOutTopic(_options.TopicPrefix, channelKey);
        ...
        public override async Task<IAsyncDisposable> SubscribeAsync(Guid channelKey, Func<ClusterFanOutMessage, CancellationToken, Task> onMessage, CancellationToken cancellationToken = default)
        {
            ...
            var loopTask = Task.Run(() => RunFanOutSubscriptionLoopAsync(topic, onMessage, loopCts.Token), loopCts.Token);
        ...
        internal async Task RunFanOutSubscriptionLoopAsync(string topic, Func<ClusterFanOutMessage, CancellationToken, Task> onMessage, CancellationToken cancellationToken)
        {
            try
            {
        ...
        {
            [LoggerMessage(EventId = 9910, Level = LogLevel.Warning,
                Message = "[Cluster] MQTT fan-out publish to topic '{Topic}' failed.")]
            public static partial void FanOutPublishFailed(ILogger logger, Exception exception, string topic);

            [LoggerMessage(EventId = 9911, Level = LogLevel.Warning,
                Message = "[Cluster] MQTT fan-out message could not be parsed; skipping it.")]
            public static partial void FanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 9912, Level = LogLevel.Error,
                Message = "[Cluster] MQTT fan-out handler faulted.")]
