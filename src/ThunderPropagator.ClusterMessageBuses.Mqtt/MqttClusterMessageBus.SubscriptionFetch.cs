...
namespace ThunderPropagator.ClusterMessageBuses.Mqtt
{
    internal sealed partial class MqttClusterMessageBus
    {
        public override async Task<IReadOnlyList<ClusterSubscriptionDescriptor>> FetchPeerSubscriptionsAsync(Uri peerEndpoint, Guid channelKey, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await SendRequestAsync(
                    peerEndpoint, MqttClusterRequestKind.FetchSubscriptions, null, channelKey, null, cancellationToken)
        ...
        private MqttClusterResponseEnvelope BuildFetchSubscriptionsResponse(MqttClusterRequestEnvelope request)
        {
            ...
            return new MqttClusterResponseEnvelope(request.CorrelationId, true, null, descriptors.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9950, Level = LogLevel.Warning,
                Message = "[Cluster] Fetching subscriptions from MQTT peer '{Host}' failed.")]
