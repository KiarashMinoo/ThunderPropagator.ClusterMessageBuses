...
{
    /// <summary>
    /// Byte-oriented counterpart to <c>MqttClusterMessageBus.FanOut.cs</c> /
    ...
    /// Fan-out uses its own per-channel topic (<see cref="MqttTopicNaming.ByteFanOutTopic"/>); the
    /// snapshot pull reuses the existing hand-rolled request/reply plumbing in
    /// <c>MqttClusterMessageBus.RequestReply.cs</c> via a new
    ...
    internal sealed partial class MqttClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterByteMessage message, CancellationToken cancellationToken = default)
        {
            ...
            var topic = MqttTopicNaming.ByteFanOutTopic(_options.TopicPrefix, channelKey);
        ...
        public override async Task<IAsyncDisposable> SubscribeAsync(Guid channelKey, Func<ClusterByteMessage, CancellationToken, Task> onMessage, CancellationToken cancellationToken = default)
        {
            ...
            var loopTask = Task.Run(() => RunByteFanOutSubscriptionLoopAsync(topic, onMessage, loopCts.Token), loopCts.Token);
        ...
        internal async Task RunByteFanOutSubscriptionLoopAsync(string topic, Func<ClusterByteMessage, CancellationToken, Task> onMessage, CancellationToken cancellationToken)
        {
            try
            {
        ...
        {
            [LoggerMessage(EventId = 9960, Level = LogLevel.Warning,
                Message = "[Cluster] MQTT byte fan-out publish to topic '{Topic}' failed.")]
            public static partial void ByteFanOutPublishFailed(ILogger logger, Exception exception, string topic);

            [LoggerMessage(EventId = 9961, Level = LogLevel.Warning,
                Message = "[Cluster] MQTT byte fan-out message could not be parsed; skipping it.")]
            public static partial void ByteFanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 9962, Level = LogLevel.Error,
                Message = "[Cluster] MQTT byte fan-out handler faulted.")]
