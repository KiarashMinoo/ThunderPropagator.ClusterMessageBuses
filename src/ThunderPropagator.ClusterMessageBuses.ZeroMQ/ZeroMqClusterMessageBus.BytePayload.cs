...
{
    /// <summary>
    /// Byte-oriented counterpart to <c>ZeroMqClusterMessageBus.FanOut.cs</c> /
    ...
    /// rides over the same DEALER→ROUTER connection as every other frame kind, discriminated by the
    ...
    /// request/reply plumbing in <c>ZeroMqClusterMessageBus.RequestReply.cs</c> via a
    ...
    internal sealed partial class ZeroMqClusterMessageBus
    {
        ...
        private static partial class Log
        {
            [LoggerMessage(EventId = 91550, Level = LogLevel.Warning,
                Message = "[Cluster] ZeroMQ byte fan-out send to peer '{Host}' failed.")]
            public static partial void ByteFanOutSendToPeerFailed(ILogger logger, Exception exception, string host);

            [LoggerMessage(EventId = 91551, Level = LogLevel.Warning,
                Message = "[Cluster] ZeroMQ byte fan-out message could not be parsed; skipping it.")]
            public static partial void ByteFanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91552, Level = LogLevel.Error,
                Message = "[Cluster] ZeroMQ byte fan-out handler faulted.")]
