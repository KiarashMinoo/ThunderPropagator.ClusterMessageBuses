...
{
    /// <summary>
    /// DI registration for the WebSocket direct-peer-connection <see cref="IClusterMessageBus"/> transport.
    /// </summary>
    public static class WebSocketClusterMessageBusExtensions
    {
        /// <summary>
        /// Registers <see cref="WebSocketClusterMessageBus"/> as the <see cref="IClusterMessageBus"/>
        /// singleton: this node runs its own WebSocket listener and dials one persistent outbound
        ...
        /// connection — see <see cref="WebSocketClusterMessageBus"/>'s remarks for the full design.
        /// </summary>
        /// <remarks>
        ...
        ///         .AddClusterWebSocketMessageBus(options => options.ListenPath = "/cluster/ws");
        /// </code>
        /// </example>
        public static IServiceCollection AddClusterWebSocketMessageBus(this IServiceCollection services, Action<WebSocketClusterMessageBusOptions> configure)
        {
            services.Configure(configure);
            services.TryAddSingleton<IClusterChannelResolver, ChannelManagerResolver>();
            services.TryAddSingleton<WebSocketClusterMessageBus>();
            services.TryAddSingleton<IClusterMessageBus>(sp => sp.GetRequiredService<WebSocketClusterMessageBus>());
