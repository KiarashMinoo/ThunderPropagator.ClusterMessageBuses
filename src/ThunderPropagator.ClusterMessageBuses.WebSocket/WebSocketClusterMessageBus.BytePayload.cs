...
{
    /// <summary>
    /// Byte-oriented counterpart to <c>WebSocketClusterMessageBus.FanOut.cs</c> /
    /// <c>.RequestReply.cs</c> -- see <see cref="ClusterByteMessage"/>'s own doc comment for why this
    /// surface exists alongside, not instead of, the <c>ClusterFanOutMessage</c>-based one. Fan-out
    /// reuses the same multiplexed connection as every other frame kind, discriminated by the
    /// <see cref="ClusterFrameKind.ByteFanOut"/> frame kind and wrapped in a
    /// <see cref="ClusterChannelEnvelope{TMessage}"/> exactly like the fan-out path does for
    /// <c>ClusterFanOutMessage</c>; the snapshot pull reuses the existing hand-rolled correlation-id
    /// request/reply plumbing in <c>WebSocketClusterMessageBus.RequestReply.cs</c> via a
    ...
    internal sealed partial class WebSocketClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterByteMessage message, CancellationToken cancellationToken = default)
        {
            ...
            await Task.WhenAll(peers.Select(async peer =>
            {
                ...
                catch (Exception exception)
                {
                    ...
                    // ClusterFanOutMessage overload in WebSocketClusterMessageBus.FanOut.cs.
        ...
        {
            [LoggerMessage(EventId = 91260, Level = LogLevel.Warning,
                Message = "[Cluster] WebSocket byte fan-out send to peer '{Host}' failed.")]
            public static partial void ByteFanOutSendToPeerFailed(ILogger logger, Exception exception, string host);

            [LoggerMessage(EventId = 91261, Level = LogLevel.Warning,
                Message = "[Cluster] WebSocket byte fan-out message could not be parsed; skipping it.")]
            public static partial void ByteFanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91262, Level = LogLevel.Error,
                Message = "[Cluster] WebSocket byte fan-out handler faulted.")]
