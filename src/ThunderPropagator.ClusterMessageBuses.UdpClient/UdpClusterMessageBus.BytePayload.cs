...
{
    /// <summary>
    /// Byte-oriented counterpart to <c>UdpClusterMessageBus.FanOut.cs</c> /
    ...
    /// Fan-out is multiplexed over the same shared socket as every other concern, using the
    ...
    /// channel key alongside a <c>ClusterFanOutMessage</c>. Delivery is still best-effort, exactly
    /// like the <c>ClusterFanOutMessage</c> overload -- UDP gives no delivery guarantee regardless of
    /// payload shape. The snapshot pull reuses the existing hand-rolled, resend-on-timer request/reply
    /// plumbing in <c>UdpClusterMessageBus.RequestReply.cs</c> via a new
    ...
    internal sealed partial class UdpClusterMessageBus
    {
        ...
        public override async Task PublishAsync(Guid channelKey, ClusterByteMessage message, CancellationToken cancellationToken = default)
        {
            ...
            await Task.WhenAll(peers.Select(async peer =>
            {
                ...
                {
                    // One unreachable/unresolvable peer must not fail the whole fan-out -- and, as
                    // with the ClusterFanOutMessage overload, even a "successful" send here is only
                    // best-effort: UDP gives no delivery guarantee at all.
        ...
        {
            [LoggerMessage(EventId = 91560, Level = LogLevel.Warning,
                Message = "[Cluster] UDP byte fan-out send to peer '{Host}' failed.")]
            public static partial void ByteFanOutSendToPeerFailed(ILogger logger, Exception exception, string host);

            [LoggerMessage(EventId = 91561, Level = LogLevel.Warning,
                Message = "[Cluster] UDP byte fan-out message could not be parsed; skipping it.")]
            public static partial void ByteFanOutMessageUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91562, Level = LogLevel.Error,
                Message = "[Cluster] UDP byte fan-out handler faulted.")]
