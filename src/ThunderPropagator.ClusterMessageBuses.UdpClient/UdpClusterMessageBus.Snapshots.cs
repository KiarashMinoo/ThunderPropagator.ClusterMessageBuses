    ...
    internal sealed partial class UdpClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, UdpClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, UdpClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);
        ...
        private async Task<UdpClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(UdpClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new UdpClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<UdpClusterResponseEnvelope> BuildSyncDeltaResponseAsync(UdpClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new UdpSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new UdpClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 91540, Level = LogLevel.Information,
                Message = "[Cluster] UDP restoring channel '{Channel}' from leader host '{Host}'.")]
            ...
            [LoggerMessage(EventId = 91541, Level = LogLevel.Information,
                Message = "[Cluster] UDP restored {Count} snapshot entries for channel '{Channel}'.")]
            ...
            [LoggerMessage(EventId = 91542, Level = LogLevel.Debug,
                Message = "[Cluster] UDP delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
