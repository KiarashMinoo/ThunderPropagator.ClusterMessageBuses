    ...
    internal sealed partial class RedisPubSubClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, RedisPubSubClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, RedisPubSubClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);

            var delta = response.PayloadJson?.FromNJson<RedisPubSubSnapshotDeltaPayload>();
            if (delta is null)
                return;
        ...
        private async Task<RedisPubSubClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(RedisPubSubClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new RedisPubSubClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<RedisPubSubClusterResponseEnvelope> BuildSyncDeltaResponseAsync(RedisPubSubClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new RedisPubSubSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new RedisPubSubClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 91140, Level = LogLevel.Information,
                Message = "[Cluster] Redis pub/sub restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91141, Level = LogLevel.Information,
                Message = "[Cluster] Redis pub/sub restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91142, Level = LogLevel.Debug,
                Message = "[Cluster] Redis pub/sub delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
