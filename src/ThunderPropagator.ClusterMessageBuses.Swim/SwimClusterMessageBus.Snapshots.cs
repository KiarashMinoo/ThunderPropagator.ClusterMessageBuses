...
namespace ThunderPropagator.ClusterMessageBuses.Swim
{
    internal sealed partial class SwimClusterMessageBus
    {
        ...
        {
            [LoggerMessage(EventId = 91640, Level = LogLevel.Information,
                Message = "[Cluster] SWIM restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91641, Level = LogLevel.Information,
                Message = "[Cluster] SWIM restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91642, Level = LogLevel.Debug,
                Message = "[Cluster] SWIM delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
