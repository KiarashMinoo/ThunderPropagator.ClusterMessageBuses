    ...
    internal sealed partial class AwsSqsClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, AwsSqsClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, AwsSqsClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);
        ...
        private async Task<AwsSqsClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(AwsSqsClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new AwsSqsClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<AwsSqsClusterResponseEnvelope> BuildSyncDeltaResponseAsync(AwsSqsClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new AwsSqsSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new AwsSqsClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9740, Level = LogLevel.Information,
                Message = "[Cluster] AwsSqs restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 9741, Level = LogLevel.Information,
                Message = "[Cluster] AwsSqs restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 9742, Level = LogLevel.Debug,
                Message = "[Cluster] AwsSqs delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
