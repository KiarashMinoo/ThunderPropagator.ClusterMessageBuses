...
namespace ThunderPropagator.ClusterMessageBuses.Srd
{
    internal sealed partial class SrdClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterSubscriptionEvent subscriptionEvent, CancellationToken cancellationToken = default)
        {
            ...
            var frame = new SrdClusterFrame(SrdClusterFrameKind.SubscriptionEvent, new ClusterChannelEnvelope<ClusterSubscriptionEvent>(channelKey, stamped).ToNJson());
        ...
        {
            [LoggerMessage(EventId = 91820, Level = LogLevel.Warning,
                Message = "[Cluster] SRD subscription-event send to peer '{Host}' failed.")]
            public static partial void SubscriptionEventSendToPeerFailed(ILogger logger, Exception exception, string host);

            [LoggerMessage(EventId = 91821, Level = LogLevel.Warning,
                Message = "[Cluster] SRD subscription event could not be parsed; skipping it.")]
            public static partial void SubscriptionEventUnparseable(ILogger logger, Exception exception);

            [LoggerMessage(EventId = 91822, Level = LogLevel.Error,
                Message = "[Cluster] SRD subscription-event handler faulted.")]
