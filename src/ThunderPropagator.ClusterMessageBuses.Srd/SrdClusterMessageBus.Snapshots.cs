...
namespace ThunderPropagator.ClusterMessageBuses.Srd
{
    internal sealed partial class SrdClusterMessageBus
    {
        ...
        {
            [LoggerMessage(EventId = 91840, Level = LogLevel.Information,
                Message = "[Cluster] SRD restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91841, Level = LogLevel.Information,
                Message = "[Cluster] SRD restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91842, Level = LogLevel.Debug,
                Message = "[Cluster] SRD delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
