    ...
    internal sealed partial class ActiveMqClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, ActiveMqClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, ActiveMqClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);

            var delta = response.PayloadJson?.FromNJson<ActiveMqSnapshotDeltaPayload>();
            if (delta is null)
                return;
        ...
        private async Task<ActiveMqClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(ActiveMqClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new ActiveMqClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<ActiveMqClusterResponseEnvelope> BuildSyncDeltaResponseAsync(ActiveMqClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new ActiveMqSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new ActiveMqClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 91040, Level = LogLevel.Information,
                Message = "[Cluster] ActiveMQ restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91041, Level = LogLevel.Information,
                Message = "[Cluster] ActiveMQ restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91042, Level = LogLevel.Debug,
                Message = "[Cluster] ActiveMQ delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
