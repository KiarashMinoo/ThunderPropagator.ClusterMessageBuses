...
namespace ThunderPropagator.ClusterMessageBuses.Kafka
{
    internal sealed partial class KafkaClusterMessageBus
    {
        public override async Task<IReadOnlyList<ClusterSubscriptionDescriptor>> FetchPeerSubscriptionsAsync(Uri peerEndpoint, Guid channelKey, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await SendRequestAsync(
                    peerEndpoint, KafkaClusterRequestKind.FetchSubscriptions, null, channelKey, null, cancellationToken)
        ...
        private KafkaClusterResponseEnvelope BuildFetchSubscriptionsResponse(KafkaClusterRequestEnvelope request)
        {
            ...
            return new KafkaClusterResponseEnvelope(request.CorrelationId, true, null, descriptors.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9550, Level = LogLevel.Warning,
                Message = "[Cluster] Fetching subscriptions from Kafka peer '{Host}' failed.")]
