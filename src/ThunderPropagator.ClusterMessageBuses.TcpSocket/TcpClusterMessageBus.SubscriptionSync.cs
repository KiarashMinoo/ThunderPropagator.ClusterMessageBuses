...
namespace ThunderPropagator.ClusterMessageBuses.TcpSocket
{
    internal sealed partial class TcpClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterSubscriptionEvent subscriptionEvent, CancellationToken cancellationToken = default)
        {
            ...
            var frame = new TcpClusterFrame(TcpClusterFrameKind.SubscriptionEvent, new TcpSubscriptionEventPayload(channelKey, stamped).ToNJson());
        ...
        {
            TcpSubscriptionEventPayload? subscriptionEventPayload;
            try
            {
                subscriptionEventPayload = payload.FromNJson<TcpSubscriptionEventPayload>();
            }
            catch (Exception exception)
            {
