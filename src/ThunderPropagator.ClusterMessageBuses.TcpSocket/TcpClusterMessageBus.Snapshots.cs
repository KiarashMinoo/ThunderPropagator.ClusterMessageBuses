    ...
    internal sealed partial class TcpClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, TcpClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, TcpClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);
        ...
        private async Task<TcpClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(TcpClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new TcpClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<TcpClusterResponseEnvelope> BuildSyncDeltaResponseAsync(TcpClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new TcpSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new TcpClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 91440, Level = LogLevel.Information,
                Message = "[Cluster] TCP restoring channel '{Channel}' from leader host '{Host}'.")]
            public static partial void RestoringChannel(ILogger logger, string channel, string host);

            [LoggerMessage(EventId = 91441, Level = LogLevel.Information,
                Message = "[Cluster] TCP restored {Count} snapshot entries for channel '{Channel}'.")]
            public static partial void RestoredChannel(ILogger logger, int count, string channel);

            [LoggerMessage(EventId = 91442, Level = LogLevel.Debug,
                Message = "[Cluster] TCP delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
