    ...
    internal sealed partial class NatsClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, NatsClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, NatsClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);

            var delta = response.PayloadJson?.FromNJson<NatsSnapshotDeltaPayload>();
            if (delta is null)
                return;
        ...
        private async Task<NatsClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(NatsClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new NatsClusterResponseEnvelope(true, null, entries.ToNJson());
        }
        ...
        private async Task<NatsClusterResponseEnvelope> BuildSyncDeltaResponseAsync(NatsClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new NatsSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new NatsClusterResponseEnvelope(true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9740, Level = LogLevel.Information,
                Message = "[Cluster] NATS restoring channel '{Channel}' from leader host '{Host}'.")]
            ...
            [LoggerMessage(EventId = 9741, Level = LogLevel.Information,
                Message = "[Cluster] NATS restored {Count} snapshot entries for channel '{Channel}'.")]
            ...
            [LoggerMessage(EventId = 9742, Level = LogLevel.Debug,
                Message = "[Cluster] NATS delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
