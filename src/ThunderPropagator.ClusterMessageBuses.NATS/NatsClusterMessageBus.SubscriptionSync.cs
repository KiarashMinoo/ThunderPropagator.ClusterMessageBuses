...
namespace ThunderPropagator.ClusterMessageBuses.NATS
{
    internal sealed partial class NatsClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterSubscriptionEvent subscriptionEvent, CancellationToken cancellationToken = default)
        {
            ...
            var subject = NatsSubjectNaming.SubscriptionEventSubject(_options.SubjectPrefix, channelKey);
            ...
            {
                Log.SubscriptionEventPublishFailed(_logger, exception, subject);
            }
        }
        ...
        public override async Task<IAsyncDisposable> SubscribeAsync(Guid channelKey, Func<ClusterSubscriptionEvent, CancellationToken, Task> onEvent, CancellationToken cancellationToken = default)
        {
            ...
            var loopTask = Task.Run(() => RunSubscriptionEventSubscriptionLoopAsync(subject, onEvent, loopCts.Token), loopCts.Token);
        ...
        internal async Task RunSubscriptionEventSubscriptionLoopAsync(string subject, Func<ClusterSubscriptionEvent, CancellationToken, Task> onEvent, CancellationToken cancellationToken)
        {
            try
            {
                await foreach (var delivery in _transport.SubscribeAsync(subject, cancellationToken).ConfigureAwait(false))
                {
                    await HandleSubscriptionEventDeliveryAsync(delivery.Payload, onEvent, cancellationToken).ConfigureAwait(false);
                }
            }
        ...
        private static partial class Log
        {
            [LoggerMessage(EventId = 9720, Level = LogLevel.Warning,
                Message = "[Cluster] NATS subscription-event publish to subject '{Subject}' failed.")]
            public static partial void SubscriptionEventPublishFailed(ILogger logger, Exception exception, string subject);

            [LoggerMessage(EventId = 9721, Level = LogLevel.Warning,
                Message = "[Cluster] NATS subscription event could not be parsed; skipping it.")]
            ...
            [LoggerMessage(EventId = 9722, Level = LogLevel.Error,
                Message = "[Cluster] NATS subscription-event handler faulted.")]
