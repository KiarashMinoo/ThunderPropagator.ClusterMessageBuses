...
namespace ThunderPropagator.ClusterMessageBuses.Pulsar
{
    internal sealed partial class PulsarClusterMessageBus
    {
        public override async Task PublishAsync(Guid channelKey, ClusterSubscriptionEvent subscriptionEvent, CancellationToken cancellationToken = default)
        {
            ...
            var topic = PulsarTopicNaming.SubscriptionEventTopic(_options.TopicPrefix, channelKey);
        ...
        public override async Task<IAsyncDisposable> SubscribeAsync(Guid channelKey, Func<ClusterSubscriptionEvent, CancellationToken, Task> onEvent, CancellationToken cancellationToken = default)
        {
            ...
            var topic = PulsarTopicNaming.SubscriptionEventTopic(_options.TopicPrefix, channelKey);
            var subscriptionName = $"{_options.SubscriptionPrefix}-subscriptions-{_subscriptionSuffix}";
            var loopCts = CancellationTokenSource.CreateLinkedTokenSource(_lifetimeCts.Token);
            var loopTask = Task.Run(() => RunSubscriptionEventSubscriptionLoopAsync(topic, subscriptionName, onEvent, loopCts.Token), loopCts.Token);
        ...
        internal async Task RunSubscriptionEventSubscriptionLoopAsync(string topic, string subscriptionName, Func<ClusterSubscriptionEvent, CancellationToken, Task> onEvent, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private static partial class Log
        {
            [LoggerMessage(EventId = 9820, Level = LogLevel.Warning,
                Message = "[Cluster] Pulsar subscription-event publish to topic '{Topic}' failed.")]
            ...
            [LoggerMessage(EventId = 9821, Level = LogLevel.Warning,
                Message = "[Cluster] Pulsar subscription event could not be parsed; skipping it.")]
            ...
            [LoggerMessage(EventId = 9822, Level = LogLevel.Error,
                Message = "[Cluster] Pulsar subscription-event handler faulted.")]
