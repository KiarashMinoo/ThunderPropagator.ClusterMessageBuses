    ...
    internal sealed partial class PulsarClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, PulsarClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, PulsarClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);

            var delta = response.PayloadJson?.FromNJson<PulsarSnapshotDeltaPayload>();
            if (delta is null)
                return;
        ...
        private async Task<PulsarClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(PulsarClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new PulsarClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<PulsarClusterResponseEnvelope> BuildSyncDeltaResponseAsync(PulsarClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new PulsarSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new PulsarClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9840, Level = LogLevel.Information,
                Message = "[Cluster] Pulsar restoring channel '{Channel}' from leader host '{Host}'.")]
            ...
            [LoggerMessage(EventId = 9841, Level = LogLevel.Information,
                Message = "[Cluster] Pulsar restored {Count} snapshot entries for channel '{Channel}'.")]
            ...
            [LoggerMessage(EventId = 9842, Level = LogLevel.Debug,
                Message = "[Cluster] Pulsar delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
