...
namespace ThunderPropagator.ClusterMessageBuses.GcpPubSub
{
    internal sealed partial class GcpPubSubClusterMessageBus
    {
        public override async Task<IReadOnlyList<ClusterSubscriptionDescriptor>> FetchPeerSubscriptionsAsync(Uri peerEndpoint, Guid channelKey, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await SendRequestAsync(
                    peerEndpoint, GcpPubSubClusterRequestKind.FetchSubscriptions, null, channelKey, null, cancellationToken)
        ...
        private GcpPubSubClusterResponseEnvelope BuildFetchSubscriptionsResponse(GcpPubSubClusterRequestEnvelope request)
        {
            ...
            return new GcpPubSubClusterResponseEnvelope(request.CorrelationId, true, null, descriptors.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9850, Level = LogLevel.Warning,
                Message = "[Cluster] Fetching subscriptions from GcpPubSub peer '{Host}' failed.")]
