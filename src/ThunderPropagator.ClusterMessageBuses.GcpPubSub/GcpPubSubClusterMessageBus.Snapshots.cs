    ...
    internal sealed partial class GcpPubSubClusterMessageBus
    {
        public override async Task RestoreFromLeaderAsync(Uri leaderEndpoint, IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            var response = await SendRequestAsync(
                leaderEndpoint, GcpPubSubClusterRequestKind.RestoreSnapshot, channel.Metadata.ChannelName, null, null, cancellationToken)
        ...
        {
            var response = await SendRequestAsync(
                leaderEndpoint, GcpPubSubClusterRequestKind.SyncDelta, channel.Metadata.ChannelName, null, since.UtcTicks, cancellationToken)
                .ConfigureAwait(false);
        ...
        private async Task<GcpPubSubClusterResponseEnvelope> BuildRestoreSnapshotResponseAsync(GcpPubSubClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            return new GcpPubSubClusterResponseEnvelope(request.CorrelationId, true, null, entries.ToNJson());
        }
        ...
        private async Task<GcpPubSubClusterResponseEnvelope> BuildSyncDeltaResponseAsync(GcpPubSubClusterRequestEnvelope request, CancellationToken cancellationToken)
        {
            ...
            var payload = new GcpPubSubSnapshotDeltaPayload { UpdatedEntries = updatedEntries, DeletedHashKeys = deletedHashKeys };
            return new GcpPubSubClusterResponseEnvelope(request.CorrelationId, true, null, payload.ToNJson());
        }

        private static partial class Log
        {
            [LoggerMessage(EventId = 9840, Level = LogLevel.Information,
                Message = "[Cluster] GcpPubSub restoring channel '{Channel}' from leader host '{Host}'.")]
            ...
            [LoggerMessage(EventId = 9841, Level = LogLevel.Information,
                Message = "[Cluster] GcpPubSub restored {Count} snapshot entries for channel '{Channel}'.")]
            ...
            [LoggerMessage(EventId = 9842, Level = LogLevel.Debug,
                Message = "[Cluster] GcpPubSub delta sync applied {Updated} updates and {Deleted} deletions for channel '{Channel}'.")]
